...
router.post('/listings',
  ...
  async (req: Request, res: Response) => {
    ...
    const images = imagePaths.length > 0 ? imagePaths : files.map((f: any) => '/' + f.path.replace(/\\/g, '/'));
    ...
    try {
      ...
      if (first?.path) {
        ...
        form.append('file', fs.createReadStream(first.path), {
          filename: first.originalname || path.basename(first.path),
          contentType: first.mimetype || 'image/jpeg'
        });

        const r = await fetch(`${config.mlServiceUrl}/infer`, {
          method: 'POST',
          body: form,
      ...
      if (qualityScore == null) {
        // This might fail if images are now URLs, but let's assume analyze handles it or we skip
        // The original code passed 'images' which were local paths. Now 'images' are Cloudinary URLs (if upload succeeded).
        // If ML service analyze endpoint needs local paths, we have a problem if we already uploaded.
        // CHECK: The original code used `images` (mapped from files) for `analyze`. 
        // `images` is now Cloudinary URLs. 
        // If `mlServiceUrl/analyze` expects local paths or URLs?
        // Let's look at controller: analyzeImageAndSuggestPrice uses local file. 
        // This fallback `metrics` call seems to depend on `images`. 
        // If we want to support this fallback, we might need to pass Cloudinary URLs and hope ML service supports it, 
        // OR rely on the fact that we still have `files` (local) available?
        // But `analyze` payload takes `images`. 
        // Let's assume for now we use the Cloudinary URLs.
    ...
    files.forEach((f: any) => {
      fs.unlink(f.path, (err) => {
        if (err) console.error('Failed to cleanup local file', f.path, err);
      });
    });
...
router.get('/listings', async (req: Request, res: Response) => {
  ...
  // Only show approved or available (legacy) listings in public feed
  if (!filter.status) {
    filter.status = { $in: ['APPROVED', 'available'] };
  }
