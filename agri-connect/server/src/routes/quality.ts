    ...
    try {
      ...
      for (const url of candidates) {
        ...
        try {
          ...
          if (r.ok) {
            const data = (await r.json()) as MLQualityResponse;
            ...
            try {
              ...
              (doc?.items || []).forEach((it: any) => {
                ...
                if (name in pricesMap && typeof it?.price === 'number') {
                  pricesMap[name] = it.price;
                }
              });

              // Prefer weighted expected price if model provided probabilities
              const scores = data.final_scores || data.vit_class?.scores;
              if (scores && typeof scores === 'object') {
                ...
                if (totalW > 0) {
                  data.suggestedPricePerKg =
                    Math.round((weighted / totalW) * 100) / 100;
                }
              }

              // Fallback: exact label match
              if (data.suggestedPricePerKg == null) {
                const label = String(data.final_quality || '').trim();
                const p = pricesMap[label];
                if (typeof p === 'number') {
                  data.suggestedPricePerKg = p;
                }
              }
            } catch {
              // pricing lookup failure should not break ML response
            }
      ...
      return res
        .status(500)
        .json({ message: 'failed to analyze image', error: err?.message || String(err) });
    }
  }
);
