...
const Nav: React.FC = () => {
  ...
  const homePath = role ? (role === 'admin' ? '/admin/dashboard' : role === 'farmer' ? '/my-listings' : '/listings') : '/'
  return (
    <div
      ...
      <div className="w-full h-14 px-6 flex items-center">
        <div className="flex items-center gap-5 flex-1 min-w-0">
          <Link to={homePath} className="flex items-center gap-2">
            <img src={Logo} alt="Agri-Connect" className="h-6 w-6" />
...
const AppInner: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <footer
        className={`${role ? 'bg-white border-t border-green-100 text-gray-700' : ''} fixed bottom-0 left-0 right-0 h-12`}
  ...
  <div className="w-full h-full px-6 flex items-center justify-between text-sm font-semibold whitespace-nowrap">
    <div className="flex items-center">
      {role && <AssistantWidget inline />}
    </div>
    <div className="flex items-center gap-10">
      <span className="text-[11px] font-semibold">© 2025 Agri-Connect. All rights reserved.</span>
